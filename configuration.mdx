...
The keymap is set with `expr = true` so the function return value (the Ex command string) is executed as the keymap RHS, which preserves dot-repeat. The `setup()` call runs first and narrows the URL pattern to HTTP/HTTPS only; the subsequent `url()` call then uses that updated pattern.
